...
app.get("/", (req, res) => {
  res.json({
    message: "Blockchain Supply Chain API is running 🚀",
    endpoints: {
      register: "POST /register",
      updateStatus: "PATCH /update-status/:id",
...
app.post("/register", async (req, res) => {
  try {
    ...
    if (role !== "Manufacturer") {
      return res
        .status(403)
        .json({ error: "Only manufacturers can register products" });
    }
  ...
  try {
    ...
    if (validNextStep[product.status] !== nextStatus) {
      return res.status(400).json({
        error: `Invalid Move. Current: ${product.status}. Expected: ${
          validNextStep[product.status]
        }`,
      });
    }
...
app.get("/verify/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: chainBroken
        ? "CRITICAL: Chain is broken. Tampering detected!"
        : "Chain is secure.",
    });
  } catch (err) {
...
app.listen(PORT, () =>
  console.log(`Blockchain Supply Chain API running on port ${PORT}`)
);
